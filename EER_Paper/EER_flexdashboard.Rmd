...
refs_com <- merge(unique(alluv_dt[,-"Window"]), Refs[! ItemID_Ref_Target %in% c("Null","0")], by = "Id", allow.cartesian = TRUE)
...
top_refs <- refs_com %>% 
  select(Leiden1, com_years, ItemID_Ref, Nom, Annee, Titre, Revue, norm_dev_ref) %>% 
  unique() %>% 
  group_by(Leiden1) %>% 
...
na_title <- na_title %>% 
  filter(! ItemID_Ref %in% top_refs$ItemID_Ref)
