...
generate_png = TRUE # choosing if you want to recreate the png of the different graphs or just use those already created
...
top_ref <- copy(Refs[ItemID_Ref_Target != "NULL", nb_cit := .N, by = "ItemID_Ref_Target"][order(-nb_cit), head(.SD, 1), .(ItemID_Ref_Target)][,.(Nom, Titre, Annee_Bibliographique_Target,nb_cit)]%>% slice_max(order_by = nb_cit, n = 20))

top_ref_alt <- merge(Refs[ItemID_Ref_Target != "NULL"], Corpus[, c("ID_Art","Annee_Bibliographique")], by.x = "ID_Art_Source", by.y = "ID_Art") 
...
top_ref_alt <- top_ref_alt[order(-nb_cit_weighted), head(.SD, 1), .(ItemID_Ref_Target)][,.(Nom, Titre, Annee_Bibliographique_Target,nb_cit_weighted)] %>% 
  slice_max(order_by = nb_cit_weighted, n = 20)

top_ref <- merge(top_ref, top_ref_alt, by = "Titre", all = TRUE)
...
setnames(top_ref, c("Nom.x","Annee_Bibliographique_Target.x"), c("Nom","Annee_Bibliographique"))
...
refs_com <- merge(unique(alluv_dt[,-"Window"]), Refs, by = "Id", allow.cartesian = TRUE)
...
top_refs <- refs_com %>% 
  select(Leiden1, com_years, Nom, Annee, Titre, Revue, norm_dev_ref) %>% 
  unique() %>% 
  group_by(Leiden1) %>% 
...
top_ref <- copy(refs_macro[ItemID_Ref_Target != "NULL", nb_cit := .N, by = "ItemID_Ref_Target"][order(-nb_cit), head(.SD, 1), .(ItemID_Ref_Target)][,.(Nom, Titre, Annee_Bibliographique_Target,nb_cit)]%>% slice_max(order_by = nb_cit, n = 20))

top_ref_alt <- merge(refs_macro[ItemID_Ref_Target != "NULL"], Corpus[, c("ID_Art","Annee_Bibliographique")], by.x = "ID_Art_Source", by.y = "ID_Art") 
top_ref_alt <- top_ref_alt[, nb_cit_year := .N, by = "Annee_Bibliographique"][,nb_cit_year := 1/nb_cit_year][, nb_cit_weighted := sum(nb_cit_year), by = "ItemID_Ref_Target"]
top_ref_alt <- top_ref_alt[order(-nb_cit_weighted), head(.SD, 1), .(ItemID_Ref_Target)][,.(Nom, Titre, Annee_Bibliographique_Target,nb_cit_weighted)] %>% 
  slice_max(order_by = nb_cit_weighted, n = 20)
...
Graphs{.tabset .tabset-pill .tabset-fade data-height=700}
-------------------------------------
