...
topic_diff <- readRDS(paste0(data_path, "EER/topic_model_1-60_topics_diff.rds"))
...
top_terms <- readRDS(paste0(data_path, "EER/topic_model_1-60_top_terms.rds")) %>%
  rename("topic_n" = topic) %>%
  filter(measure == "frex")

# merge institutions data for EER and top5
institutions_eer <- readRDS(paste0(data_path, "EER/1_Corpus_Prepped_and_Merged/Institutions_cleaned.rds")) %>%
...
```{r}
knitr::include_graphics(paste0(here::here(), "/EER_Paper/Pictures/mean_diff_plot1-60.png"))
```
