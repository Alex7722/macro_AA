...
# Decades
Corpus <- Corpus[, decade:= cut(Annee_Bibliographique, seq(1969, 2025, 8), right = FALSE, dig.lab = 4)]
...
nodes_1 <- tbl %>% activate(nodes) %>% as.data.table()
...
nodes_2 <- tbl %>% activate(nodes) %>% as.data.table()
...
JEL <- readRDS("/projects/data/macro_AA/Corpus_Econlit_Matched_WoS/Old_JEL_matched_corpus_nodes.rds")
jel_nodes <- tbl %>% activate(nodes) %>% as.data.table()
jel_nodes <- jel_nodes[,JEL_id:=0][ID_Art %in% JEL$ID_Art, JEL_id:=1]

# write.csv(jel_nodes[JEL_id==0 & (Leiden_num == 10 | Leiden_num== 3 | Leiden_num== 2 | Leiden_num== 4 | Leiden_num== 6)],"EER/check_jel.csv", row.names = FALSE)
# jel_nodes <- merge(jel_nodes, JEL[,.(ID_Art, JEL_id=1)], by.x = "ID_Art", all.x = TRUE)

jel_nodes <- jel_nodes[,n_tot_com:=.N, .(Leiden1)]
jel_nodes <- jel_nodes[,share:=sum(JEL_id)/n_tot_com*100, .(Leiden1)]
jel_nodes <- jel_nodes[order(-share), head(.SD, 1), .(Leiden1)]
jel_nodes <- jel_nodes[,count_with_share := paste0(Leiden1,"(", round(share),"%",")"), .(Leiden1)]
jel_nodes <- jel_nodes[,.(Proportion_JEL_codes = toString(count_with_share)), Leiden1]

summary <- Reduce(merge, list(tfidf_table, most_cited_nodes, most_cited_refs, most_cited_disc, most_prod_journal, most_prod_author, most_prod_institutions, most_prod_countries, collaborations, jel_nodes))
...
nodes_long <- rbind(nodes_1[,head(.SD,1), ID_Art][,time_frame:=1][,.(ID_Art, x, y, time_frame, size, color, Label)], 
                    nodes_2[,head(.SD,1), ID_Art][,time_frame:=2][,.(ID_Art, x, y, time_frame, size, color, Label)])

nodes_long <- nodes_long[order(ID_Art)]

nodes_long <- melt(nodes_long, id.vars=c("ID_Art", "x", "y", "size", "color", "Label"),  measure.vars=c("time_frame"))


# nodes_long <- nodes_long[value==2,x:=x+(sample(50:250, 100))][value==2,y:=y+(sample(50:250, 100))]
library(plotly)
library(gapminder)
...
tbl_list <- make_a_graph(tbl_coup_list[[6]], color, 1, "RBConfigurationVertexPartition", treshold_color=0.1, 0.02, layout_graph = "TRUE", 10000,2000)
tbl_list$components
tbl <- tbl_list$tbl
