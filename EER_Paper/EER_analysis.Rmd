...
```{r echo=FALSE,  message=FALSE, warning=FALSE, error=FALSE, results=TRUE, out.width='100%'}
kable(Refs[ItemID_Ref_Target!="NULL"][order(-nb_cit_tot)][, head(.SD, 1), .(Label_Target)][,.(Label_Target, Titre, nb_cit_tot)] %>% top_n(50)) %>% kable_styling(bootstrap_options = c("striped", "condensed", full_width = F))
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, error=FALSE, results=TRUE, out.width='100%'}
kable(Refs[ItemID_Ref_Target!="NULL"][order(-nb_cit_tot)][, head(.SD, 1), .(Label_Target, ESpecialite_custom)][order(ESpecialite_custom), head(.SD, 5), .(ESpecialite_custom)][,.(ESpecialite_custom, Label_Target, Titre, nb_cit_tot)]) %>% kable_styling(bootstrap_options = c("striped", "condensed", full_width = F))
...
ggplot(disc_ref_year[ESpecialite_custom!="Economics"], aes(x=as.numeric(Annee_Bibliographique_Source), y=share_years_mean, group=ESpecialite_custom, color=ESpecialite_custom, weight=weight)) +
  ...
  scale_color_discrete(guide = FALSE) +
  geom_label_repel(aes(label = ESpecialite_custom), data = disc_ref_year[ESpecialite_custom!="Economics"][Annee_Bibliographique_Source==2019], nudge_x = 1, segment.color = NA) +
  theme_minimal() +
