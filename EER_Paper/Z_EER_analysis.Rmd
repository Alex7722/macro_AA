...
ggplot(mean_jel[V1!=0 & Annee_Bibliographique!="2016"], aes(x=as.numeric(Annee_Bibliographique), y=V1)) +
  ...
  theme_minimal() 
  ggsave("EER/Graphs/mean_jel.png", width=35, height=20, units = "cm")
...
ggplot(count_year, aes(x=as.numeric(Annee_Bibliographique), y=freq, group=Countries_grouped, color=Countries_grouped)) +
  ...
  ggrepel::geom_label_repel(aes(label = Countries_grouped), data = count_year[Annee_Bibliographique==2019], nudge_x = 1, segment.color = NA) +
  coord_cartesian(xlim = c(NA, 2022)) 
  # ggsave("Graphs/country_time_stacked.png", scale = 1)


count_year <- Institutions[, head(.SD, 1), .(ID_Art)][Annee_Bibliographique<=2007]
...
ggplot(count_year, aes(x=as.numeric(Annee_Bibliographique), y=freq, group=EU_US_collab, color=EU_US_collab)) +
  ...
  labs(fill = "Countries (Top 7)") +
  scale_x_continuous("Years") +
  scale_y_continuous("Share of Papers Authored by European and American Economists", limits = c(0,1), breaks = seq(0,1,0.2)) +
  theme_minimal() +
  scale_fill_manual(values = c(rev(brewer.pal(n = 9, name = "Paired")))) +
  scale_color_discrete(guide = FALSE) +
  ggrepel::geom_label_repel(aes(label = EU_US_collab), data = count_year[Annee_Bibliographique==2007], nudge_x = 1, segment.color = NA) +
  coord_cartesian(xlim = c(NA, 2014)) 
  ggsave("EER/Graphs/Collab_strict.png", width=35, height=20, units = "cm")
...
require(dplyr)
count_year <- institutions_unique[, head(.SD, 1), .(ID_Art)][Annee_Bibliographique<=2007]
count_year <- count_year %>% group_by(Annee_Bibliographique, EU_US_collab) %>% summarise(n = n()) %>%  mutate(freq = n / sum(n)) %>% as.data.table()
...
ggplot(count_year, aes(x=as.numeric(Annee_Bibliographique), y=freq, group=EU_US_collab, color=EU_US_collab)) +
  ...
  ggrepel::geom_label_repel(aes(label = EU_US_collab), data = count_year[Annee_Bibliographique==2007], nudge_x = 1, segment.color = NA) +
  coord_cartesian(xlim = c(NA, 2014)) 
  ggsave("EER/Graphs/Collab_non_strict.png", width=35, height=20, units = "cm")
...
bridges_collab <- institutions_unique[EU_US_collab== "Collaborations"][,list(Target = rep(Institution[1:(length(Institution)-1)],(length(Institution)-1):1),
...
bridges_collab <- institutions_unique[EU_US_collab== "Collaborations"][,list(Target =
...
ggplot(eco_ref_year, aes(x=as.numeric(Annee_Bibliographique_Source), y=n_eco_ref/n_ref_year, weight=weight)) +
  ...
  coord_cartesian(ylim = c(0,1)) +
  theme_minimal() 
  ggsave("EER/Graphs/coc.png", width=35, height=20, units = "cm")
