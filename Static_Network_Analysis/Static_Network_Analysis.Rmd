...
if(i >= 4){
  ...
  coupling_com <- merge(coupling_com, main_country, by = c("Com_ID","rank"), all.x = TRUE)
}
...
```{r, fig.height= 10, fig.width=15}
# plotting the heatmap with the dendrogram
heatmap <- clustering_communities(graph_authors_coupling_community)

png(paste0(picture_path,"heatmap_authors_coupling_",start_date[i],"-",end_date[i],".png"), width = 1200, height = 1000)
grid.newpage()
print(heatmap$heatmap, vp = viewport(x = 0.4, y = 0.5, width = 0.75, height = 1.0))
print(heatmap$dendrogram, vp = viewport(x = 0.87, y = 0.5, width = 0.2, height = 0.9))
dev.off()

include_graphics(paste0(picture_path,"heatmap_authors_coupling_",start_date[i],"-",end_date[i],".png"))
```
...
main_journal <- main_journal[, share_journal := .N, by = c("Com_ID","Revue")][, size := .N, by = "Com_ID"][, share_journal:= share_journal/size]
main_journal <- main_journal %>%
  select(Com_ID,Revue,share_journal) %>%
  unique() %>%
  group_by(Com_ID) %>%
  arrange(desc(share_journal)) %>%
  slice(1:n) %>%
...
authors_coupling_com <- cbind(authors_coupling_com,most_cited_articles[, c("citing_title","Revue","nb_cit")],
                              most_cited_ref[, c("cited_author","Annee","Revue_Abbrege","cited_title","share_cit_ref")], main_journal[, c("main_journal","share_journal")])
...
include_graphics(paste0(picture_path,"Sankey_diagram_Leiden_",start_date[i],"-",end_date[i],".png"))
