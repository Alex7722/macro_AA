...
i = 6 # choosing the period to study
...
include_graphics(paste0(picture_path,"graph_coupling_",start_date[i],"-",end_date[i],".png"))
```
...
doublons <- which(duplicated(most_cited_ref[,c("Com_ID","New_id2","cited_author","Annee","Revue_Abbrege")]))
...
coupling_com <- cbind(coupling_com,most_cited_nodes[, c("Label","Titre","nb_cit")],most_cited_ref[, c("cited_author","Annee","Revue_Abbrege","Cited_Title","share_cit_ref")], 
...
png(paste0(picture_path,"heatmap_coupling_",start_date[i],"-",end_date[i],".png"), width = 1200, height = 1000)
grid.newpage()
...
include_graphics(paste0(picture_path,"heatmap_coupling_",start_date[i],"-",end_date[i],".png"))
```
...
plot_graph_journal <- ggraph(graph_journal, "manual", x = x, y = y) + 
  ...
  #scale_size_continuous(range = c(1,4)) +
  theme_void() +
  ggsave(paste0(picture_path,"graph_revue_",start_date[i],"-",end_date[i],".png"), width=30, height=30, units = "cm")

include_graphics(paste0(picture_path,"graph_revue_",start_date[i],"-",end_date[i],".png"))
```
...
include_graphics(paste0(picture_path,"graph_authors_coupling_",start_date[i],"-",end_date[i],".png"))
```
...
include_graphics(paste0(picture_path,"graph_authors_coupling_community_",start_date[i],"-",end_date[i],".png"))
```
...
Nodes_authors_coupling <- merge(Nodes_authors_coupling,nodes_JEL[between(Annee_Bibliographique,start_date[i],end_date[i]) ,c("ID_Art","Annee_Bibliographique","Revue","ESpecialite")], by = "ID_Art")
...
include_graphics(paste0(picture_path,"graph_institutions_coupling_",start_date[i],"-",end_date[i],".png"))
}
...
include_graphics(paste0(picture_path,"graph_cocit",start_date[i],"-",end_date[i],".png"))
```
...
png(paste0(picture_path,"heatmap_cocit_",start_date[i],"-",end_date[i],".png"), width = 1200, height = 1000)
grid.newpage()
print(heatmap$heatmap, vp = viewport(x = 0.4, y = 0.5, width = 0.75, height = 1.0))
print(heatmap$dendrogram, vp = viewport(x = 0.87, y = 0.5, width = 0.2, height = 0.9))
dev.off()

include_graphics(paste0(picture_path,"heatmap_cocit_",start_date[i],"-",end_date[i],".png"))
```
...
include_graphics(paste0(picture_path,"graph_coauthorship_",start_date[i],"-",end_date[i],".png"))
...
include_graphics(paste0(picture_path,"graph_co-authorship_institutions_",start_date[i],"-",end_date[i],".png"))
}
```
...
# merging the cocit nodes with the papers which cite them
nodes_authors <- merge(nodes_authors, authors_JEL[between(Annee_Bibliographique, start_date[i],end_date[i]) & Nom %in% nodes_authors$Id, c("Nom","ID_Art")], by.x = "Id", by.y = "Nom") 
