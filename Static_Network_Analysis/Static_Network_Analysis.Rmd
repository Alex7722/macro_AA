...
TF_IDF <- tf_idf(nodes = Nodes_authors_coupling, n_columns = 3)
...
# Keeping the n nodes with the highest number of citations in our corpus, per community
most_cited_nodes <- unique(Nodes_cocit[, c("Com_ID","nb_cit","Label","Titre")])
most_cited_nodes <- most_cited_nodes %>%
  group_by(Com_ID) %>%
  arrange(desc(nb_cit)) %>%
  slice(1:n) %>%
...
cocit_com <- merge(cocit_com,most_cited_nodes[,c("Com_ID","rank","Label","Titre","nb_cit")], by = c("Com_ID","rank"), all.x = TRUE)
...
nodes_cocit <- merge(nodes_cocit, edges_JEL[between(Annee_Bibliographique, 1991,1999) & New_id2 %in% nodes_cocit$Id, c("New_id2","ID_Art")], by.x = "Id", by.y = "New_id2") 
...
ggplot(sankey_data, aes(x, id = id, split = y, value = value)) +
  geom_parallel_sets(aes(fill = as.character(Com_ID_cocit)), alpha = 0.5, axis.width = 0.1, show.legend = FALSE) +
  geom_parallel_sets_axes(aes(fill = y), axis.width = 0.3, show.legend = FALSE) +
  scale_fill_manual(values = c(palette_coupling$color_coupling,palette_cocit$color_cocit)) +
  theme_void() +
...
# merging the cocit nodes with the papers which cite them
nodes_authors <- merge(nodes_authors, authors_JEL[between(Annee_Bibliographique, 1991,1999) & Nom %in% nodes_authors$Id, c("Nom","ID_Art")], by.x = "Id", by.y = "Nom") 
...
# Merging the coupling and cocit data
nodes_coupling$Id <- as.integer(nodes_coupling$Id)
sankey_data <- merge(nodes_authors,nodes_coupling, by.x="ID_Art", by.y = "Id")
...
# plotting the graph
ggplot(sankey_data, aes(x, id = id, split = y, value = value)) +
  geom_parallel_sets(aes(fill = as.character(Com_ID_author)), alpha = 0.5, axis.width = 0.1, show.legend = FALSE) +
  geom_parallel_sets_axes(aes(fill = y), axis.width = 0.3, show.legend = FALSE) +
  scale_fill_manual(values = c(palette_coupling$color_coupling,palette_author$color_author)) +
  theme_void() +
  geom_parallel_sets_labels(colour = 'black', fontface = "bold", angle = 0, size = 4)
