...
for(i in 1:length(start_date)){
  ...
  # Renaming columns
  Com <- graph_coupling %>% activate(nodes) %>% select(Id,Com_ID,Community_name,Size_com,color) %>% filter(Size_com > 0.01) %>% as.data.table()
  Com$Community_name <- gsub(".*-","",Com$Community_name)
  setnames(Com, c("Com_ID","Community_name","Size_com","color"), c(paste0("Com_ID_",resolution_set[j]),
