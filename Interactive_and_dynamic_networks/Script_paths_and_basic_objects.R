...
cran_list <-  c("data.table","magrittr", "ggnewscale", "igraph",
                   "tm","quanteda","tidytext","ggraph","tidygraph", "ggrepel",
                   "reticulate","leidenAlg","reshape2","scales","scico",
                   "ggforce","directlabels","patchwork","DescTools","DT","grid","ggdendro",
                   "ggalluvial","sigmajs","knitr","tidyverse","htmltools","crosstalk",
                   "gridExtra")
for(p in cran_list){
...
picture_path <- "~/macro_AA/Interactive_and_dynamic_networks/Pictures/"
...
color_com <- data.table("Titre_provisoire" = sort(unique(community_names$Titre_provisoire)), 
