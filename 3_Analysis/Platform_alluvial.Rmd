...
In this section, you will find graphic representations of all networks and communities as alluvial diagrams. You can click on images to zoom and explore the diagrams with more precision.
...
-   Each rectangle is one cluster in a given network detected by the Leiden algorithm. Rectangles are given a color according to the intertemporal naming procedure explained in the technical documentation. Colors in the alluvial directly match colors in the graph representation on the platform.

-   The "flow" between rectangles (the alluvium) is the share of nodes going from one cluster to the other. On the upper part of each rectangles, the absence of flow on the right is the share of nodes that are not found in the previous time window network (articles published on the last year of the time window), while the absence of flow on the left is the share of nodes that disappear in the following time window network (articles published in the first year of the time window).
...
include_graphics(here(picture_path,"alluvial_platform_graph_noname.png"))

```

# List of the main macroeconomics clusters in the alluvial

In this section, you will find a number of synthetic information and tables about [**intertemporal clusters**]{.underline} (i.e. all articles in all time windows in individual clusters as identified by sub-names).
...
[**IMPORTANT DISCLAIMER**]{.underline}: because the number of articles (nodes) increase rapidly over time, communities that exists over many time windows are mostly characterized in the following tables by the latter time windows. In turn, the following tables can hide heterogeneity and great transformations within a particular cluster simply because the weight of the latter time window is simply to important. [The best exploratory tool remain to look at individual cluster on the platform for different time window]{.underline}.
